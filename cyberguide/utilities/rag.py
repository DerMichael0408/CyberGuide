import chromadb
import PyPDF2  # Replaced fitz with PyPDF2
import json
import os
...
# Go up one level to get to the cyberguide directory
cyberguide_dir = os.path.dirname(current_dir)

# Define paths relative to the cyberguide directory
DB_PATH = os.path.join(cyberguide_dir, "cybersecurity_db")
PETRA_PDF_PATH = os.path.join(cyberguide_dir, "Petra_logistics.pdf")
SCENARIOS_JSON_PATH = os.path.join(cyberguide_dir, "CybersecurityScenarios.json")
...
# Index the documents - use the full paths
try:
    index_data(PETRA_PDF_PATH)
    index_data(SCENARIOS_JSON_PATH)
except Exception as e:
    print(f"Warning: Could not index documents: {e}")
    print("The application will still run, but RAG features may not work properly.")
